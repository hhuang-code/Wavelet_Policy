...
import agents.models.wavelet.utils as utils
...
class Wavelet_Policy(nn.Module):
    def __init__(self, model: DictConfig, obs_encoder: DictConfig, visual_input: bool = False, device: str = "cpu", **kwargs):
        ...
        for num_epoch in tqdm(range(self.epoch)):
            ...
            with utils.eval_mode(self.action_ae):
                for data in self.train_dataloader:
                    ...
                    observations = self.scaler.scale_input(observations)    # (b, win, ob_dim=4)
                    action = self.scaler.scale_output(action)               # (b, win, act_dim=2)
