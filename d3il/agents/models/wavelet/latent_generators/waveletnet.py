...
import einops
import agents.models.wavelet.latent_generators.latent_generator as latent_generator

import agents.models.wavelet.libraries.waveletnet.model as waveletnet_model
import agents.models.wavelet.libraries.waveletnet.trainer as waveletnet_trainer
from agents.models.wavelet.libraries.loss_fn import FocalLoss, soft_cross_entropy

from typing import Optional, Tuple


class WaveletNet(latent_generator.AbstractLatentGenerator):
    def __init__(
        self,
        input_dim: int,
        n_layer: int = 12,
        ...
        **kwargs
        ...
        wavelet_config = waveletnet_model.WaveletConfig(
        ...
        self.model = waveletnet_model.WAVELET(wavelet_config)
    ...
    def get_latent_and_loss(
        ...
        if self.predict_offsets:
            target_latents, target_offsets = target_latents     # (b, w, 1), integer; (b, w, d), float
        ...
        if self.predict_offsets:
            # todo
            output, _, d_loss, s_loss = self.model(obs_rep)     # input: (b, ws, obs_dim); output: (b, ws, n_bins + n_bins * act_dim)
            logits = output[:, :, : self.vocab_size]    # (b, ws, n_bins)
            ...
            loss = offset_loss + class_loss + self.detail_loss_weight * d_loss + self.smooth_loss_weight * s_loss
            ...
            if return_loss_components:
                return (
                    (logits, offsets),
                    loss,
                    {"offset": offset_loss, "class": class_loss, "detail_loss": d_loss, "smooth_loss": s_loss, "total": loss},
                )
            else:
                return (logits, offsets), loss
        else:
            logits, _, d_loss, s_loss = self.model(obs_rep)
            loss = criterion(logits.view(-1, logits.size(-1)), target_latents)
            loss = loss + self.detail_loss_weight * d_loss + self.smooth_loss_weight * s_loss
            ...
            if return_loss_components:
                return logits, loss, {"class": loss, "detail_loss": d_loss, "smooth_loss": s_loss, "total": loss}
            else:
                return logits, loss
    ...
    def generate_latents(
        ...
        output, _, _, _ = self.model(obs_rep, None)
        if self.predict_offsets:
    ...
    def get_optimizer(
        ...
        trainer_cfg = waveletnet_trainer.TrainerConfig(
