...
class Sorting_Sim(BaseSim):
    ...
    def eval_agent(self, agent, contexts, n_trajectories, mode_encoding, successes, mean_distance, pid, cpu_set):
        ...
        # env = Sorting_Env(max_steps_per_episode=self.max_steps_per_episode, render=self.render,
        env = Sorting_Env(max_steps_per_episode=self.max_steps_per_episode, render=self.render, image_path=image_path,   # Added by Hao
                          num_boxes=self.num_box, if_vision=self.if_vision)
        env.start()
