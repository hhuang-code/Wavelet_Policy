...
class Stacking_Sim(BaseSim):
    ...
    def eval_agent(self, agent, contexts, n_trajectories, mode_encoding, mode_encoding_1_box, mode_encoding_2_box,
        ...
        # Added by Hao -----------------------------------------------
        image_dir = os.path.join(agent.working_dir, 'video')
        if not os.path.isdir(image_dir):
            os.makedirs(image_dir)
        image_path = os.path.join(image_dir, 'frame_%07d.png')
        # ------------------------------------------------------------

        # env = CubeStacking_Env(max_steps_per_episode=self.max_steps_per_episode, render=self.render)
        env = CubeStacking_Env(max_steps_per_episode=self.max_steps_per_episode, render=self.render, image_path=image_path)   # Added by Hao
        env.start()
    ...
    def test_agent(self, agent):
        ...
        print(f'Entropy {entropy_3}')
        print(f'Entropy_1 {entropy_1}')
        print(f'Entropy_2 {entropy_2}')

        log.info(f'Successrate {success_rate}')
        log.info(f'Successrate_1 {box1_success_rate}')
        log.info(f'Successrate_2 {box2_success_rate}')

        log.info(f'Entropy {entropy_3}')
