...
class Avoiding_Sim(BaseSim):
    ...
    def eval_agent(self, agent, n_trajectories, mode_encoding, successes, robot_c_pos, pid, cpu_set):
        ...
        env = ObstacleAvoidanceEnv(render=self.render, image_path=image_path)   # Added by Hao
        env.start()
        ...
        for i in range(n_trajectories):
            ...
            while not done:
                obs = np.concatenate((pred_action[:2], obs))    # (actual position, desired position)

                pred_action = agent.predict(obs)    # desired velocity: (1, act_dim)
            ...
            c_pos = torch.tensor(np.array(c_pos))[:, :2][:150, :]    # Added by Hao
