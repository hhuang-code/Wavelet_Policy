...
class ObstacleAvoidanceEnv(GymEnvWrapper):
    def __init__(
            ...
            debug: bool = False,
            render: bool = False,
            image_path: str = None, # Added by Hao
    ):

        self.render = render    # Added by Hao
        ...
        scene = sim_factory.create_scene(
            ...
            object_list=obj_list, render=render_mode, dt=0.001, image_path=image_path,  # Added by Hao
        )
        robot = MjRobot(
            scene,
    ...
    def step(self, action, gripper_width=None):
        ...
        if self.render:
            img = np.zeros(
                (
                    glfw.get_framebuffer_size(self.scene.viewer.windows[0])[1],
                    glfw.get_framebuffer_size(self.scene.viewer.windows[0])[0],
                    3,
                ),
                dtype=np.uint8,
            )
            mujoco.mjr_readPixels(img, None, self.scene.viewer.viewport, self.scene.viewer.con)
            imageio.imwrite(self.scene.viewer._image_path % self.scene.viewer._image_idx, np.flipud(img))
            self.scene.viewer._image_idx += 1
        # --------------------------------------------------------------------
    ...
    def reset(self, random=True, context=None):
        ...
        if self.render:
            ...
            folder = Path(os.path.dirname(self.scene.viewer._image_path)).expanduser().resolve()
            png_files = [p for p in folder.glob('*.png') if p.is_file() and p.suffix.lower() == ".png"]

            if len(png_files) > 0:
                # Use a context-manager so the writer is always closed
                video_name = os.path.join(os.path.dirname(self.scene.viewer._image_path),
                                          f'avoiding_{self.episode}.mp4')
                with imageio.get_writer(video_name, fps=4) as writer:
                    for p in png_files:
                        frame = imageio.v2.imread(p)  # ndarray (H, W, 3) or (H, W, 4)
                        writer.append_data(frame)

                for f in png_files:
                    f.unlink(missing_ok=True)

            if self.scene.viewer._image_idx != 0:
                self.scene.viewer._image_idx = 0
        # --------------------------------------------------------------------
