...
class Avoiding_Dataset(TrajectoryDataset):
    def __init__(
        ...
        # shape: B, T, n
        self.observations = torch.from_numpy(np.concatenate(inputs)).to(device).float()     # (96, 200, 4)
        self.actions = torch.from_numpy(np.concatenate(actions)).to(device).float()         # (96, 200, 2)
        self.masks = torch.from_numpy(np.concatenate(masks)).to(device).float()             # (96, 200)

        self.num_data = len(self.observations)

        self.slices = self.get_slices() # len = 6921 with win_sz = 5
