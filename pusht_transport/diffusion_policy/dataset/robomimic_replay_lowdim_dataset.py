...
class RobomimicReplayLowdimDataset(BaseLowdimDataset):
    def __init__(self,
        ...
        with h5py.File(dataset_path) as file:
            demos = file['data']    # <HDF5 group "/data" (200 members)>
            for i in tqdm(range(len(demos)), desc="Loading hdf5 to ReplayBuffer"):
                demo = demos[f'demo_{i}']
                episode = _data_to_obs(
                    raw_obs=demo['obs'],    # (T=714, d=59)
                    raw_actions=demo['actions'][:].astype(np.float32),  # (T=714, d=20)
                    obs_keys=obs_keys,
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        torch_data = dict_apply(data, torch.from_numpy) # obs: (T=10, 59); actions: (T=10, 20)
