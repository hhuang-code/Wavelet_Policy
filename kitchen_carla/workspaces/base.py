...
class Workspace:
    def __init__(self, cfg):
        ...
        if 'waveletnet' in cfg.experiment:
        ...
        wandb.init(dir=self.work_dir, project=cfg.project, config=cfg._content, name=os.path.basename(str(self.work_dir)))
        self.epoch = 0
        self.load_snapshot()
    ...
    def _get_action(self, obs, sample=False, keep_last_bins=False):
        with utils.eval_mode(
            ...
            # print(f'{np.array(enc_obs_seq.shape).tolist()}, {np.array(latents.shape).tolist()}, '
            #       f'{np.array(action_latents[0].shape).tolist()}, {np.array(action_latents[1].shape).tolist()}')
            if sample:
