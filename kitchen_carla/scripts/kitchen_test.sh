...
export MUJOCO_GL=glfw  # Comment out when using HPC
...
for subfolder in "${folder}"/*; do
    ...
    if [[ -d "${subfolder}" ]] && [[ "${subfolder}" == *"train"* ]] && [[ "${subfolder}" == *"kitchen"* ]] && [[ "${subfolder}" == *"$model"* ]]; then
        ...
        find "${subfolder}" -type f -name "snapshot_*.pt" | while read -r file; do
            ...
            python run_on_env.py --config-name=eval_kitchen state_prior=waveletnet_kitchen_best \
            project=wavelet_policy_kitchen experiment=kitchen_eval_wavelet \
