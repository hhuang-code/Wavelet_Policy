...
class RelayKitchenTrajectoryDataset(TensorDataset):
    def __init__(self, data_directory, device="cpu"):
        ...
        observations = np.load(data_directory / "observations_seq.npy")     # (T=409, N=566, d=60)
        actions = np.load(data_directory / "actions_seq.npy")   # (T=409, N=566, d=9)
        masks = np.load(data_directory / "existence_mask.npy")  # (T=409, N=566)
        ...
        self.actions = self.tensors[1]  # (N=566, T=409, d=9)
...
class CarlaMultipathTrajectoryDataset(Dataset):
    def __init__(
        ...
        self.observations_all = torch.load(self.data_directory / "all_observations.pth") # (N=100, T=614, 3, 224, 224)
        self.actions_all = torch.load(self.data_directory / "all_actions_pm1.pth")  # (N=100, T=614, d=2)
...
class PushTrajectoryDataset(TensorDataset):
    def __init__(
        ...
        self.observations = np.load(self.data_directory / "multimodal_push_observations.npy")   # (N=1000, T=201, d=16)
        self.actions = np.load(self.data_directory / "multimodal_push_actions.npy") # (N=1000, T=201, d=2)
        self.masks = np.load(self.data_directory / "multimodal_push_masks.npy")     # (N=1000, T=201)
