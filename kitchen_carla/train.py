...
class Workspace:
    def __init__(self, cfg):
        ...
        if 'wavelet' in cfg.experiment:
            # new_work_dir = self.work_dir.with_name(self.work_dir.name + f'_dw{cfg.detail_loss_weight}_sw{cfg.smooth_loss_weight}')
            new_work_dir = self.work_dir.with_name(self.work_dir.name)# + f'_dw{cfg.detail_loss_weight}_sw{cfg.smooth_loss_weight}')
            os.rename(self.work_dir, new_work_dir)
        ...
        # kitchen: get_relay_kitchen_train_val
        ...
        self.wandb_run = wandb.init(
            ...
            # name=os.path.basename(str(self.work_dir)),
            name=self.work_dir.parent.name + f'_{self.work_dir.name}',
        )
        wandb.config.update(
            {
    ...
    def train_prior(self):
        ...
        with utils.eval_mode(self.obs_encoding_net, self.action_ae):
            ...
            for data in pbar:
                observations, action, mask = data   # (b, ws, obs_dim), (b, ws, act_dim), (b, ws)
                self.state_prior_optimizer.zero_grad(set_to_none=True)
                obs, act = observations.to(self.device), action.to(self.device)
                enc_obs = self.obs_encoding_net(obs)    # same shape as obs
                latent = self.action_ae.encode_into_latent(act, enc_obs)    # (b, ws, 1), (b, ws, act_dim)
    ...
    def run(self):
        ...
        )   # self.action_ae.bin_centers: (n_bins, d=9)
        ...
        if self.cfg.lazy_init_models:
            self._init_state_prior()
        self.state_prior_iterator = tqdm.trange(self.prior_epoch, self.cfg.num_prior_epochs)
        ...
        for epoch in self.state_prior_iterator:
            ...
            if ((self.prior_epoch + 1) % self.cfg.save_prior_every) == 0:
                if self.prior_epoch - 1 >= self.cfg.num_prior_epochs - 5:  # save the last five checkpoints
    ...
    def save_snapshot(self, epoch):
        ...
        with Path(str(self.snapshot).replace('.pt', f'_{epoch}.pt')).open('wb') as f:
